...
/***/ function(module, exports, __webpack_require__) {
	...
	var DiffCanvas = __webpack_require__(1);
	var DiffType = DiffCanvas.DiffType;
	var DiffAlgorithm = __webpack_require__(5);
	var basic = DiffAlgorithm.basic;
	var ResultStack = __webpack_require__(7);
	var DiffTable = __webpack_require__(8);
	var ButtonGetReady = __webpack_require__(9);
	var config = __webpack_require__(2);

	var oldStr = config.oldStr, newStr = config.newStr;

	// var scrollToLatest = document.getElementById('scrollToLatest');
	// // OneDiff.toScrollToLastestLog = scrollToLatest.checked;
	// scrollToLatest.addEventListener('change', function() {
	//   // OneDiff.toScrollToLastestLog = this.checked;
	// }, false);
	...
	__webpack_require__(4)(this, $);
	var oldStr = "abcabba", newStr = "cbabac";
	var basicDiffCanvas = null, forwardDiffCanvas = null, reverseDiffCanvas = null, globalCanvas = null, diffTable = null, resultStack = null;
	...
	function init(curDiffAlgorithm, toOverwrite) {
	  ...
	  if (curDiffAlgorithm == 'basic') {
	    ...
	    diffTable = new DiffTable('#forward-table', 1, oldStr.length + newStr.length);
	    diffTable.modify([{
	      "row": 0,
	      "col": 0,
	      "value": 0
	    ...
	    reverseDiffCanvas = new DiffCanvas('#' + tabsInfo[1].id, reverseStr(oldStr), reverseStr(newStr));
	    globalCanvas = new DiffCanvas('#' + globalTabInfo.id, oldStr, newStr);
	    diffTable = new DiffTable('#forward-table', 2, oldStr.length + newStr.length);
	    resultStack = new ResultStack(oldStr, newStr, {
	...
	document.getElementById('ok').addEventListener('click', function() {
	  oldStr = document.getElementById('string1').value;
	  newStr = document.getElementById('string2').value;
	  var diffAlgorithmBtns = document.getElementsByName('diffAlgorithm');
	  diffAlgorithmBtns.forEach(function(oneBtn) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	DiffCanvas.prototype = {
	  ...
	  "drawArrows": function(oneDiff, color) {
	    if (oneDiff.type == this.DiffType.removed) this.drawHorizontalArrow(oneDiff, color);
	    else if (oneDiff.type == this.DiffType.common) this.drawDiagonalArrow(oneDiff, color);
	    else if (oneDiff.type == this.DiffType.added) this.drawVerticalArrow(oneDiff, color);
	  },
	  ...
	  "drawEditPath": function(editPath) {
	    for (var i in editPath) this.drawDiff(editPath[i], 'red');
	    return false;
	  },
	  "undrawEditPath": function(editPath) {
	    if (0 == editPath.length) return false;
	    for (var i in editPath) this.undrawDiff(editPath[i]);
	    return true;
	  },
	  ...
	  "drawSquares": function(ranges) {
	    ...
	    if (ranges.length == undefined) ranges = new Array(ranges);
	    for (var i in ranges) this.drawSquare(ranges[i], parseInt(i) ? 'black' : 'red');
	  }
	};
	    ...
	    module.exports = function( root, jQuery ) {
	      if ( jQuery === undefined ) {
	        ...
	        if ( typeof window !== 'undefined' ) {
	          ...
	          jQuery = __webpack_require__(3)(root);
	        }
	      }
	      factory(jQuery);
	      return jQuery;
	  ...
	  +function ($) {
	    ...
	    Tooltip.prototype.getCalculatedOffset = function (placement, pos, actualWidth, actualHeight) {
	      return placement == 'bottom' ? { top: pos.top + pos.height,   left: pos.left + pos.width / 2 - actualWidth / 2 } :
	             ...
	             placement == 'mouse' ? { top: window.mousePos.y - actualHeight, left: window.mousePos.x - actualWidth } :
...
/***/ function(module, exports, __webpack_require__) {
	...
	var DiffCanvas = __webpack_require__(1);
	var DiffType = DiffCanvas.DiffType;
	var OneDiff = __webpack_require__(6);
	...
	// OffsetArr.prototype.constructor = OffsetArr;
	...
	// OffsetTable.prototype.constructor = OffsetTable;

	function OneScene(range, before, after, config) {
	  this.before = before;
	  this.after = after;
	  this.range = range;
	  if (config) {
	    this.tableChange = config.tableChange;
	  }
	}
	OneScene.prototype = {
	  "constructor": OneScene,
	...
	function OneTableChange(type, config) {
	  this.type = type;
	  if (config) {
	    if (type == OneTableChange.prototype.type['new']) {
	      this.d = config.d;
	      this.after = config.after;
	    } else if (type == OneTableChange.prototype.type.change) {
	      this.changes = config;
	    }
	  }
	}
	OneTableChange.prototype = {
	  "constructor": OneTableChange,
	  "type": {
	    "new": 0,
	    "change": 1
	  },
	...
	var DiffAlgorithm = {
	  "basic": {
	    "getResult": function(before, after) {
	      var info = this.basicInfo(before, after);
	      info.result.push(this.getEditPathWithDMax(before, after, info.DMax));
	      return info.result;
	    },
	    "getEditPath": function(before, after) {
	      return this.getEditPathWithDMax(before, after, this.basicInfo(before, after).DMax);
	    },
	    "basicInfo": function(before, after) {
	      ...
	      if (beforeLength == 0) {
	        for (var i = 0; i < afterLength; ++i) result.push(new OneDiff(0, i, DiffType.added, parseInt(i) + 1));
	        return {
	          "result": result,
	          "DMax": afterLength
	        };
	      } else if (afterLength == 0) {
	        for (var i = 0; i < beforeLength; ++i) result.push(new OneDiff(i, 0, DiffType.removed, parseInt(i) + 1));
	        return {
	          "result": result,
	          "DMax": beforeLength
	        };
	      ...
	      var pathX = new OffsetArr(DMax, -1);
	      pathX.set(1, 0);
	      for (var D = 0; D <= DMax; ++D) {
	        for (var k = -D + kStartOffset; k <= D - kEndOffset; k += 2) {
	          var newX = null, modifyingDiff = null, removed = false;
	          if (k == -D || (k != D && pathX.at(k - 1) < pathX.at(k + 1))) {
	            newX = pathX.at(k + 1);
	            if (D) result.push(modifyingDiff = new OneDiff(newX, newX - k - 1, DiffType.added, D));
	          } else {
	            newX = pathX.at(k - 1) + 1, removed = true;
	            result.push(modifyingDiff = new OneDiff(newX - 1, newX - k, DiffType.removed, D));
	          }
	          var newY = newX - k;
	          ...
	          var changes = [{
	                ...
	                "col": k,
	                "value": newX,
	                "class": [ OneTableChange.prototype.class.selected ]
	              }];
	          if (k != -D) changes.push({
	                "row": 0,
	                "col": k - 1,
	                "class": [ OneTableChange.prototype.class.choice, (removed ? OneTableChange.prototype.class.selected : '') ]
	              });
	          if (k != D) changes.push({
	                "row": 0,
	                "col": k + 1,
	          ...
	          if (D) modifyingDiff['tableChange'] = new OneTableChange(OneTableChange.prototype.type.change, changes);
	          

	          while (newX < beforeLength && newY < afterLength && before[newX] == after[newY]) {
	            result.push(new OneDiff(newX, newY, DiffType.common, D, {
	              "tableChange": new OneTableChange(OneTableChange.prototype.type.change, [{
	                "row": 0,
	                "col": k,
	                "value": newX + 1,
	                "class": [ OneTableChange.prototype.class.selected ]
	    ...
	    "getEditPathWithDMax": function(before, after, DMax) {
	      var beforeLength = before.length;
	      var afterLength = after.length;
	      var editPath = new Array();
	      if (beforeLength == 0) {
	        for (var i = 0; i < afterLength; ++i) editPath.push(new OneDiff(0, i, DiffType.added, 1));
	        return editPath;
	      } else if (afterLength == 0) {
	        for (var i = 0; i < beforeLength; ++i) editPath.push(new OneDiff(i, 0, DiffType.removed, 1));
	        return editPath;
	      }
	      var kStartOffset = 0, kEndOffset = 0;
	      var pathXs = new OffsetTable(DMax, DMax, -1);
	      pathXs.set(-1, 1, 0);
	      for (var D = 0; D <= DMax; ++D) {
	        for (var k = -D + kStartOffset; k <= D - kEndOffset; k += 2) {
	          var newX = null;
	          if (k == -D || (k != D && pathXs.at(D - 1, k - 1) < pathXs.at(D - 1, k + 1))) {
	            newX = pathXs.at(D - 1, k + 1);
	          } else {
	            newX = pathXs.at(D - 1, k - 1) + 1;
	          }
	          var newY = newX - k;
	          ...
	          if (newX > beforeLength) kEndOffset += 2;
	            ...
	            for (var index = DMax; index >= 0; --index) {
	              ...
	              while (0 < newX && 0 < newY && before[newX - 1] == after[newY - 1]) {
	                if (index) {
	                  rightward = (-DMax <= k - 1 && pathXs.at(index - 1, k - 1) + 1 == newX);
	                  downward = (k + 1 <= DMax && pathXs.at(index - 1, k + 1) == newX);
	                  if (rightward || downward) break;
	                }
	                --newX, --newY;
	                editPath.push(new OneDiff(newX, newY, DiffType.common, 1));
	              }
	              if (index == 0) break;
	              rightward = -DMax <= k - 1 && pathXs.at(index - 1, k - 1) + 1 == newX;
	              downward = k + 1 <= DMax && pathXs.at(index - 1, k + 1) == newX;
	              if (downward) {
	                ++k;
	                editPath.push(new OneDiff(newX, newX - k, DiffType.added, 1));
	              } else if (rightward) {
	                --k, --newX;
	                editPath.push(new OneDiff(newX, newX - k, DiffType.removed, 1));
	              }
	            }
	            editPath.reverse();
	            return editPath;
	      ...
	      throw new Error("failed to get edit path");
	    }
	  },
	  "advanced": {
	    "get": function(before, after) {
	      var ret = new Array();
	      this.getResult(ret, before, after, {
	        "before": {
	          "begin": 0,
	          "end": before.length
	        },
	    ...
	    "getResult": function(result, before, after, range) {
	      result.push(new OneScene(range, before, after, {
	        "tableChange": new OneTableChange(OneTableChange.prototype.type['new'], {
	          "d": before.length + after.length,
	          "after": new OneTableChange(OneTableChange.prototype.type.change, [{
	            "row": 0,
	            "col": 0,
	            "value": 0
	          }, {
	      ...
	      this.getDiffs(result, before, after, range);
	    },
	    "getDiffs": function(result, before, after, range) {
	      var beforeLength = before.length;
	      var afterLength = after.length;
	      if (beforeLength == 0) {
	          if (afterLength) {
	            for (var i = 0; i < afterLength; ++i) result.push(new OneDiff(0, i, DiffType.added, parseInt(i) + 1, {
	              "globalOneDiff": new OneDiff(range.before.begin + 0, range.after.begin + i, DiffType.added, parseInt(i) + 1)
	            }));
	          }
	          return result;
	      } else if (afterLength == 0) {
	          for (var i = 0; i < beforeLength; ++i) result.push(new OneDiff(i, 0, DiffType.removed, parseInt(i) + 1, {
	            "globalOneDiff": new OneDiff(range.before.begin + i, range.after.begin + 0, DiffType.removed, parseInt(i) + 1)
	          }));
	          return result;
	      } else if (beforeLength == afterLength && beforeLength == 1) {
	          if (before[0] == after[0]) result.push(new OneDiff(0, 0, DiffType.common, 1, {
	            "globalOneDiff": new OneDiff(range.before.begin + 0, range.after.begin + 0, DiffType.common, 1)
	          }));
	          else result.push(new OneDiff(0, 0, DiffType.removed, 1, {
	            "globalOneDiff": new OneDiff(range.before.begin + 0, range.after.begin + 0, DiffType.removed, 1)
	          })), result.push(new OneDiff(1, 0, DiffType.added, 1, {
	            "globalOneDiff": new OneDiff(range.before.begin + 1, range.after.begin + 0, DiffType.added, 1)
	          }));
	          return result;
	      } else if (before == after) {
	          for (var i = 0; i < beforeLength; ++i) result.push(new OneDiff(i, i, DiffType.common, 0, {
	            "globalOneDiff": new OneDiff(range.before.begin + i, range.after.begin + i, DiffType.common, 0)
	          }));
	          return result;
	      }
	      var pre = this.hasCommonPrefix(before, after);
	      if (pre) {
	        result.push(new OneScene({
	          "before": {
	            "begin": range.before.begin,
	            "end": range.before.begin + pre
	          },
	          "after": {
	            "begin": range.after.begin,
	            "end": range.after.begin + pre
	          }
	        }, before.substring(0, pre), after.substring(0, pre)));
	        for (var i = 0; i < pre; ++i) result.push(new OneDiff(i, i, DiffType.common, parseInt(i) + 1, {
	          "globalOneDiff": new OneDiff(range.before.begin + i, range.after.begin + i, DiffType.common, parseInt(i) + 1)
	        }));
	        var partB = this.getResult(result, before.substring(pre), after.substring(pre), {
	          "before": {
	            "begin": range.before.begin + pre,
	            "end": range.before.end
	          },
	          "after": {
	            "begin": range.after.begin + pre,
	            "end": range.after.end
	          }
	        });
	        // result.concat(partB);
	      ...
	      var suf = this.hasCommonSuffix(before, after);
	      if (suf) {
	        this.getResult(result, before.substring(0, beforeLength - suf), after.substring(0, afterLength - suf), {
	          "before": {
	            "begin": range.before.begin,
	            "end": range.before.end - suf
	          },
	          "after": {
	            "begin": range.after.begin,
	            "end": range.after.end - suf
	          }
	        });
	        result.push(new OneScene({
	          "before": {
	            "begin": range.before.end - suf,
	            "end": range.before.end
	          },
	          "after": {
	            "begin": range.after.end - suf,
	            "end": range.after.end
	          }
	        }, before.substring(beforeLength - suf), after.substring(afterLength - suf)));
	        for (var i = 0; i < suf; ++i) result.push(new OneDiff(i, i, DiffType.common, 0, {
	          "globalOneDiff": new OneDiff(range.before.end - suf + i, range.after.end - suf + i, DiffType.common, 0)
	        }));
	        return result;
	      }
	      var delta = beforeLength - afterLength;
	      var DoubleDMax = beforeLength + afterLength;
	      var forwardCheck = delta & 1;
	      var DMax = (DoubleDMax + 1) / 2;
	      var kForwardStartOffset = 0, kForwardEndOffset = 0;
	      var kReverseStartOffset = 0, kReverseEndOffset = 0;
	      pathXs = new OffsetTable(1, DoubleDMax, -1);
	      pathXs.set(0, 1, 0), pathXs.set(1, 1, 0);
	      for (var D = 0; D < DMax; ++D) {
	        for (var k = -D + kForwardStartOffset; k <= D - kForwardEndOffset; k += 2) {
	          var newX = null;
	          var modifyingDiff = null, removed = false;
	          if (k == -D || (k != D && pathXs.at(0, k - 1) < pathXs.at(0, k + 1))) {
	            newX = pathXs.at(0, k + 1);
	            if (D) result.push(modifyingDiff = new OneDiff(newX, newX - k - 1, DiffType.added, D));
	          } else {
	            newX = pathXs.at(0, k - 1) + 1, removed = true;
	            result.push(modifyingDiff = new OneDiff(newX - 1, newX - k, DiffType.removed, D));
	          }
	          var newY = newX - k;
	          var changes = [{
	          ...
	          while (newX < beforeLength && newY < afterLength
	                 && before[newX] == after[newY]) {
	            result.push(new OneDiff(newX, newY, DiffType.common, D, {
	              "tableChange": new OneTableChange(OneTableChange.prototype.type.change, [{
	                "row": 0,
	                "col": k,
	          ...
	          if (newX > beforeLength) kForwardEndOffset += 2;
	          else if (newY > afterLength) kForwardStartOffset += 2;
	          var reverseK = delta - k;
	          if (forwardCheck && ((reverseK < 0) ? -reverseK : reverseK) <= DoubleDMax && -1 != pathXs.at(1, reverseK)) {
	            var forwardX = beforeLength - pathXs.at(1, reverseK);
	            
	            if (newX >= forwardX) {
	              pathXs = null;
	              this.getResult(result, before.substring(0, newX), after.substring(0, newY), {
	                "before": {
	                  "begin": range.before.begin,
	              ...
	              var partB = this.getResult(result, before.substring(newX), after.substring(newY), {
	                "before": {
	        ...
	        for (var k = -D + kReverseStartOffset; k <= D - kReverseEndOffset; k += 2) {
	          var newX = null;
	          var modifyingDiff = null, removed = false;
	          if (k == -D || (k != D && pathXs.at(1, k - 1) < pathXs.at(1, k + 1))) {
	            newX = pathXs.at(1, k + 1);
	            if (D) result.push(modifyingDiff = new OneDiff(beforeLength - newX, afterLength - newX + k, DiffType.added, D, {
	              "reverse": true
	            }));
	          } else {
	            newX = pathXs.at(1, k - 1) + 1, removed = true;
	            result.push(modifyingDiff = new OneDiff(beforeLength - newX, afterLength - newX + k, DiffType.removed, D, {
	              "reverse": true
	            }));
	          }
	          var newY = newX - k;
	          ...
	          while (newX < beforeLength && newY < afterLength
	                 && before[beforeLength - newX - 1] == after[afterLength - newY - 1]) {
	            result.push(new OneDiff(beforeLength - 1 - newX, afterLength - 1 - newY, DiffType.common, D, {
	              "reverse": true,
	              "tableChange": new OneTableChange(OneTableChange.prototype.type.change, [{
	                "row": 1,
	                "col": k,
	          ...
	          if (newX > beforeLength) kReverseEndOffset += 2;
	          else if (newY > afterLength) kReverseStartOffset += 2;
	          var forwardK = delta - k;
	          if (!forwardCheck && ((forwardK < 0) ? -forwardK : forwardK) <= DoubleDMax && -1 != pathXs.at(0, forwardK)) {
	            var reverseX = beforeLength - pathXs.at(0, forwardK);
	            if (newX >= reverseX) {
	              var xFirst = beforeLength - reverseX, yFirst = beforeLength - reverseX - forwardK;
	              pathXs = null;
	              this.getResult(result, before.substring(0, beforeLength - reverseX), after.substring(0, beforeLength - reverseX - forwardK), {
	                "before": {
	                  "begin": range.before.begin,
	                  "end": range.before.begin + xFirst
	                },
	                "after": {
	                  "begin": range.after.begin,
	                  "end": range.after.begin + yFirst
	                }
	              });
	              var partB = this.getResult(result, before.substring(beforeLength - reverseX), after.substring(beforeLength - reverseX - forwardK), {
	                "before": {
	                  "begin": range.before.begin + xFirst,
	                  "end": range.before.end
	                },
	                "after": {
	                  "begin": range.after.begin + yFirst,
	                  "end": range.after.end
	                }
	              });
	              // result.concat(partB);
	      ...
	      for (var j = 0; j < afterLength; ++j) result.push(new OneDiff(i, j, DiffType.added, parseInt(i) + j, {
	        "globalOneDiff": new OneDiff(range.before.begin + i, range.after.begin + j, DiffType.added, parseInt(i) + 1)
	      }));
	        pathXs = null;
	      return result;
	    },
...
/***/ function(module, exports, __webpack_require__) {
	...
	function OneDiff(x, y, type, d, config) {
	  this.x = x;
	  this.y = y;
	  this.type = type;
	  this.d = d;
	  if (config) {
	    ...
	    this.globalOneDiff = config.globalOneDiff;
	    this.tableChange = config.tableChange;
	  }
	}
	OneDiff.prototype = {
...
/***/ function(module, exports, __webpack_require__) {
	...
	function BasicResultStack(oldStr, newStr, diffCanvas, diffTable) {
	  ...
	  this['result'].forEach(function(one) {
	    if (one.tableChange) one.tableChange['prev'] = prevTableChange, prevTableChange = one.tableChange;
	  });
	...
	BasicResultStack.prototype = {
	  "push": function() {
	    ...
	    var whatToDraw = this.result[this.indexToDraw];
	    if (whatToDraw.tableChange) {
	      this.diffTable.applyChange(whatToDraw.tableChange);
	    }
	    if (this.indexToDraw == this.result.length - 1) return this.diffCanvas.drawEditPath(this.result[this.indexToDraw++]);
	    else return this.diffCanvas.drawDiff(this.result[this.indexToDraw++]);
	  },
	  "pop": function() {
	    if (this.indexToDraw == 0) return false;    // unable to pop now

	    if (this.indexToDraw == this.result.length) return this.diffCanvas.undrawEditPath(this.result[--this.indexToDraw]);
	    var whatToUndraw = this.result[this.indexToDraw - 1];
	    if (whatToUndraw.tableChange) {
	      this.diffTable.recoverChangeFrom(whatToUndraw.tableChange);
	    }
	    if (this.diffCanvas.undrawDiff(this.result[this.indexToDraw - 1])) return --this.indexToDraw;  // pop successfully: see whether is it able to pop once more
	    else return true;  // failed to pop: able to pop once again
	  },
	  "clear": function() {
	    while (this.pop());
	  }
	};
	BasicResultStack.prototype.constructor = BasicResultStack;

	function toReverse(oldStrLength, newStrLength, originResult) {
	  return originResult.map(function(one, index, self) {
	    var ret = null;
	    if (one.range) {
	      oldStrLength = one.before.length;
	      newStrLength = one.after.length;
	      ret = {
	        "before": one.before.split('').reverse().join(''),
	        "after": one.after.split('').reverse().join(''),
	        "range": {
	          "before": {
	            "begin": one.range.before.end,
	            "end": one.range.before.begin
	          },
	          "after": {
	            "begin": one.range.after.end,
	            "end": one.range.after.begin
	          }
	        }
	      }
	    } else {
	      ret = {
	        "x": oldStrLength - one.x - (one.type != DiffType.added),
	        "y": newStrLength - one.y - (one.type != DiffType.removed),
	        "type": one.type,
	        "d": one.d,
	        "reverse": one.reverse
	      };
	...
	SceneStack.prototype = {
	  "push": function() {
	    return this.storage[this.indexToDraw++];
	  },
	  "pop": function() {
	    var ret = this.storage[--this.indexToDraw - 1];
	    return ret === undefined ? { "squareRanges": undefined } : ret;
	  }
	};
	SceneStack.prototype.constructor = SceneStack;

	function AdvancedResultStack(oldStr, newStr, canvases, diffTable) {
	  var res = this['result'] = DiffAlgorithm.advanced.get(oldStr, newStr);
	  ...
	  res.forEach(function(one) {
	    if (one.range) scenes.push(one);
	    if (one.tableChange) one.tableChange['prev'] = prevTableChange, prevTableChange = one.tableChange;
	  });
	  this['sceneStack'] = new SceneStack(scenes);
	  // this['tableChangeStack'] = new Array();
	  this['reverse'] = {
	    "result": toReverse(oldStr.length, newStr.length, res),
	...
	AdvancedResultStack.prototype = {
	  "push": function() {
	    var whatToDraw = this.result[this.indexToDraw];
	    var whatToDrawR = this.reverse.result[this.indexToDraw];
	    if (this.indexToDraw == this.result.length) return false;  // unable to push now
	    if (whatToDraw.tableChange) {
	      this.diffTable.applyChange(whatToDraw.tableChange);
	    }
	    ...
	    if (whatToDraw.range) {
	      this.globalCanvas.drawSquares(whatToDraw.squareRanges);
	      this.sceneStack.push();
	      if (whatToDraw['$curBg'] === undefined) {
	        var bgs = this.diffCanvas.$init(whatToDraw.before, whatToDraw.after);
	        whatToDraw['$oldBg'] = bgs.$oldBg;
	        whatToDraw['$curBg'] = bgs.$curBg;
	      } else {
	        whatToDraw['$oldBg'] = this.diffCanvas.restore(whatToDraw['$curBg']);
	      }
	      if (whatToDrawR['$curBg'] === undefined) {
	        var bgs = this.reverse.diffCanvas.$init(whatToDrawR.before, whatToDrawR.after);
	        whatToDrawR['$oldBg'] = bgs.$oldBg;
	        whatToDrawR['$curBg'] = bgs.$curBg;
	      } else {
	        whatToDrawR['$oldBg'] = this.reverse.diffCanvas.restore(whatToDrawR['$curBg']);
	      }
	    } else {
	      this.diffCanvas.drawDiff(whatToDraw, whatToDraw.reverse ? 'red' : 'black');
	      this.reverse.diffCanvas.drawDiff(whatToDrawR, whatToDrawR.reverse ? 'red' : 'black');
	      if (whatToDraw.globalOneDiff) this.globalCanvas.drawDiff(whatToDraw.globalOneDiff, 'red');
	    }
	  ...
	  "pop": function() {
	    if (this.indexToDraw == 0) return false;    // unable to pop now
	    var whatToUndraw = this.result[this.indexToDraw - 1];
	    var whatToUndrawR = this.reverse.result[this.indexToDraw - 1];
	    if (whatToUndraw.tableChange) {
	      this.diffTable.recoverChangeFrom(whatToUndraw.tableChange);
	    }
	    if (whatToUndraw.globalOneDiff) {
	      this.globalCanvas.undrawDiff(whatToUndraw.globalOneDiff);
	    }
	    if (whatToUndraw.range) {
	      var prevScene = this.sceneStack.pop();

	      this.globalCanvas.drawSquares(prevScene.squareRanges);
	      if (whatToUndraw.$oldBg) {
	        this.diffCanvas.restore(whatToUndraw.$oldBg);
	        this.reverse.diffCanvas.restore(whatToUndrawR.$oldBg);
	      }
	    ...
	    // else if (this.indexToDraw == this.result.length) return this.diffCanvas.undrawEditPath(this.result[--this.indexToDraw]);
	    else if (this.diffCanvas.undrawDiff(whatToUndraw) && this.reverse.diffCanvas.undrawDiff(whatToUndrawR)) return --this.indexToDraw;  // pop successfully: see whether is it able to pop once more
	    else return true;  // failed to pop: able to pop once again
	  },
	  "clear": function() {
	    while (this.pop());
	...
	AdvancedResultStack.prototype.constructor = AdvancedResultStack;
	...
	ResultStack.prototype.constructor = ResultStack;
	...
	DiffTable.prototype = {
	  ...
	  "modify": function(changes, notChangeValue, notBackupValue) {
	    ...
	    for (var i in changes) {
	      ...
	      var $td = $rows.eq(oneChange.row).find('td').eq(parseInt(oneChange.col) + this.d + 1);
	  ...
	  "unmodify": function(changes, notChangeValue, notChangeStyle) {
	    var $rows = $(this.selector).find('.diff-table tbody tr');
	    for (var i in changes) {
	      var $td = $rows.eq(changes[i].row).find('td').eq(parseInt(changes[i].col) + this.d + 1);
	      if (!notChangeValue && changes[i].prevValue) $td.text(changes[i].prevValue);
	      if (changes[i].class) $td.removeClass(changes[i].class.join(' '));
	    }
	  },
	  "highlightScope": function(top) {
	    var $rows = $(this.selector).find('.diff-table tbody tr');
	    $rows.eq(top).addClass('highlighted-row highlighted-row-top');
	    $rows.eq(top + 1).addClass('highlighted-row highlighted-row-bottom');
	  },
	  "unhighlightScope": function(top) {
	    var $rows = $(this.selector).find('.diff-table tbody tr');
	    $rows.eq(top).removeClass('highlighted-row highlighted-row-top');
	    $rows.eq(top + 1).removeClass('highlighted-row highlighted-row-bottom');
	  }
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ButtonGetReady = {
	  ...
	  "auto": function(resultStack) {
	    ...
	    autoShowId = setInterval(function() {
	      if (false == resultStack.push()) {
	        return console.log('It was the last step.'), clearInterval(autoShowId), autoShowId = null;
	      }
	    }, interval);
	  }
	};
